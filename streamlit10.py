...
/* Make the chat-window use the full iframe height (so iframe controls how large the box appears).
   This keeps all previous behavior (auto-scroll via the iframe script) but makes the visible
   chat area larger. */
.chat-window {
  background: var(--bg);
  border-radius: 10px;
  padding: 12px;
  height: 100%;           /* let the iframe height determine visible box size */
  box-sizing: border-box;
  overflow-y: auto;       /* inner scrolling still allowed if content overflows iframe */
...
def render_chat_iframe(messages: List[Dict[str, Any]], iframe_height: int = 720):
    rows = []
    ...
    # Keep the auto-scroll MutationObserver and load-time scroll behavior.
    full = f"""
    <!doctype html>
    <html>
    <head>
    ...
    """
    # Render iframe with a larger default height. Parent page will show a single scrollbar (if needed).
...
if not st.session_state.loaded_phone:
    ...
    # Increase base iframe height so the chat box appears larger.
    # We still grow slightly with the number of messages, but start bigger.
    content_height = 600 + num_messages * 18
    iframe_height = min(max(content_height, 600), 2500)  # cap to reasonable max to avoid huge pages
    render_chat_iframe(st.session_state.messages, iframe_height=iframe_height)

    # ---------------- SEND MESSAGE ----------------
    st.markdown("---")
    with st.form("send-message", clear_on_submit=True):
        outgoing = st.text_area("Type your message", height=100, max_chars=1600, key="outgoing_text")
